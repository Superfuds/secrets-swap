...
set +o xtrace
SECRETS=$(sed -n 's/.*"s3keyring:\(.*\)"/\1/p' deployment/"${environment}"/${app}-deployment.yaml)
...
do
  ...
  sed -ie "s/s3keyring:$SECRET/$PLAIN_SECRET/g" deployment/"${environment}"/${app}-deployment.yaml
done
set -o xtrace
